...
namespace pleos {
    ...
    void Graphic::Graphic_Texture::load_last_texture(){
        if(a_texture_displaying == Texture_Displaying::TD_Fill) {
            ...
            else if(a_last_texture.get() == 0 || a_last_texture.get()->height() != needed_height || a_last_texture.get()->width() != needed_width){
    ...
    // Returns the source to a XML text
    std::string Graphic::Graphic_Texture_Plane::to_xml_text_object_name(){return std::string("plane");}
    ...
    void Graphic::Graphic_Text::draw_on_image(std::shared_ptr<scls::__Image_Base> image) {
        if(a_text_image_generator.get() == 0){scls::print(std::string("PLEOS Graphic Text \"") + name() + std::string("\""), std::string("Badly created Graphic_Text, use the Graphic::new_text method indead."));return;}
    ...
    void Graphic::Graphic_Text::final_content(std::shared_ptr<scls::XML_Text_Base>& one_balise) {
        // Basic datas
        std::vector<scls::XML_Attribute>& attributes = one_balise.get()->xml_balise_attributes();
        ...
        if(one_balise.get()->xml_balise_name() == std::string("env")) {
            ...
            // Handle precision
            std::vector<std::string> final_text_cutted = scls::cut_string(final_text, std::string("."));
            if(precision == 0) {final_text = final_text_cutted.at(0);}
            else {
                if(final_text_cutted.size() >= 2){for(int i = 0;i<static_cast<int>(2 - final_text_cutted.at(1).size());i++){final_text += std::string("0");}}
                else{std::string to_add = std::string(".");for(int i = 0;i<precision;i++){to_add += std::string("0");}final_text+=to_add;}
            }

            // Set the text
            one_balise = scls::xml(window_struct()->balises_shared_ptr(), final_text);
        }
    }
    std::shared_ptr<scls::XML_Text_Base> Graphic::Graphic_Text::final_content() {
        if(graphic()->balises() == 0){std::shared_ptr<scls::XML_Text_Base> to_return = scls::xml(std::make_shared<scls::__Balise_Container>(), content());return to_return;}

        // Create the text
        std::shared_ptr<scls::XML_Text_Base> to_return = scls::xml(graphic()->balises_shared_ptr(), content());
        if(to_return.get()->sub_texts().size() == 0) {final_content(to_return);}
        else {
            to_return.get()->set_xml_balise_datas(graphic()->balises()->defined_balise_shared_ptr("p"));

            // Analyse each sub-text
            for(int i = 0;i<static_cast<int>(to_return.get()->sub_texts().size());i++) {final_content(to_return.get()->sub_texts()[i]);}
        }
    ...
    std::string Graphic::Graphic_Text::to_xml_text(){return std::string("<") + to_xml_text_base() + to_xml_text_background_color() + to_xml_text_color() + to_xml_text_font_size() + to_xml_text_font_size_in_scale() + to_xml_text_alignment_horizontal() + std::string(">") + content() + std::string("</") + to_xml_text_object_name() + std::string(">");};
    std::string Graphic::Graphic_Text::to_xml_text_alignment_horizontal(){if(alignment_horizontal() == scls::Alignment_Horizontal::H_Left){return std::string();}return std::string(" alignment_horizontal=center");}
    ...
    // Graphic constructor
    Graphic::Graphic(){a_style.set_background_color(scls::Color(0, 0, 0, 0));load_balises_pleos(a_balises);}
    ...
    __Graphic_Object_Base::Action_Function* Graphic::action_function_by_name(std::string needed_name){for(int i = 0;i<static_cast<int>(a_actions_functions.size());i++){if(a_actions_functions.at(i).get()->function_name == needed_name){return a_actions_functions.at(i).get();}}return 0;}
    std::shared_ptr<__Graphic_Object_Base::Action_Function> Graphic::action_function_by_name_shared_ptr(std::string needed_name){for(int i = 0;i<static_cast<int>(a_actions_functions.size());i++){if(a_actions_functions.at(i).get()->function_name == needed_name){return a_actions_functions.at(i);}}return std::shared_ptr<__Graphic_Object_Base::Action_Function>();}
    ...
    std::string Graphic::graphic_function_fragment_shader(scls::__Formula needed_formula) {
        ...
        // Function
        std::string needed_glsl = needed_formula.polynomial()->to_glsl(0);
    ...
    void Graphic::__new_form_xml(Form_2D* needed_form_2d, std::shared_ptr<scls::XML_Text_Base> xml) {
    ...
    std::shared_ptr<Form_2D> Graphic::new_triangle(std::string name, scls::Fraction x_1, scls::Fraction y_1, scls::Fraction x_2, scls::Fraction y_2, scls::Fraction x_3, scls::Fraction y_3) {
        ...
        to_return.get()->add_point(new_point(name + std::string("-p1"), x_1.to_double(), y_1.to_double()));
        to_return.get()->add_point(new_point(name + std::string("-p2"), x_2.to_double(), y_2.to_double()));
        to_return.get()->add_point(new_point(name + std::string("-p3"), x_3.to_double(), y_3.to_double()));
        add_form(to_return);
        return to_return;
    }
    ...
    std::shared_ptr<__Graphic_Object_Base> Graphic::object_by_name_shared_ptr(std::string name) {
        ...
        for(std::size_t i = 0;i<a_objects.size();i++){if(!a_objects.at(i).get()->should_delete() && a_objects.at(i).get()->name() == name){return a_objects.at(i);}}
    ...
    void Graphic::set_image_size(int image_width, int image_height){
        ...
        if(height() != -1) {a_graphic_base.get()->a_pixel_by_case_y = static_cast<double>(image_height) / height();}
        if(width() != -1) {set_pixel_by_case_x(static_cast<double>(image_width) / width());}

        // Set the base
        update_base();
    }
    // Sets the middle of the base
    void Graphic::set_middle(double middle_x, double middle_y){
        a_graphic_base.get()->a_x_middle = middle_x;
        a_graphic_base.get()->a_y_middle = middle_y;
        a_graphic_base.get()->set_x_middle_in_canonical_base(middle_x);
        a_graphic_base.get()->set_y_middle_in_canonical_base(middle_y);
        update_base();
    };
    // Sets the scale of the base
    void Graphic::set_scale(double width, double height, bool width_used, bool height_used){
        a_graphic_base.get()->a_height = height;
        a_graphic_base.get()->a_height_used = height_used;
        a_graphic_base.get()->a_width = width;
        a_graphic_base.get()->a_width_used = width_used;
        ...
        if(points_value == std::string("all")) {for(int i = 0;i<static_cast<int>(points().size());i++){needed_form->add_point(points().at(i));}}
            ...
            for(int i = 0;i<static_cast<int>(cutted.size());i++){
                ...
                if(f.error == SCLS_FUNCTION_CALLED_TEXT_NOT_FUNCTION) {
                    ...
                    if(needed_point.get() != 0){needed_form->add_point_external(needed_point);}
                ...
                else if(f.name == std::string_view("new")) {
					double x = scls::Fraction::from_std_string(f.parameters.at(0)).to_double();
					double y = scls::Fraction::from_std_string(f.parameters.at(1)).to_double();
                	std::shared_ptr<Graphic_Base_Object> b = new_object<Graphic_Base_Object>(needed_form->name() + std::string("_") + f.parameters.at(0) + std::string("-") + f.parameters.at(1));
					b.get()->set_x(x);b.get()->set_y(y);needed_form->add_point_external(b);
				}
            }
        }
    };
    ...
    std::shared_ptr<Point_2D> Graphic::new_point(std::string name, __Graphic_Object_Base* parent, double x, double y){std::shared_ptr<Point_2D>needed=std::make_shared<Point_2D>(graphic_base_shared_ptr(), name, x, y);needed.get()->set_this_object(needed);needed.get()->set_parent(parent);needed.get()->set_type(Vector_Type::VT_Point);add_point(needed);return needed;};
    std::shared_ptr<Point_2D> Graphic::new_point(std::string name, double x, double y){std::shared_ptr<Point_2D>needed=std::make_shared<Point_2D>(graphic_base_shared_ptr(), name, x, y);needed.get()->set_this_object(needed);needed.get()->set_type(Vector_Type::VT_Point);add_point(needed);return needed;};
    ...
    void Graphic::set_background_object(__Graphic_Object_Base* object) {
        // Assert
        if(a_objects.at(0).get() == object){return;}

        std::shared_ptr<__Graphic_Object_Base> object_shared_ptr;
        for(int i = 0;i<static_cast<int>(a_objects.size());i++){if(a_objects.at(i).get() == object){object_shared_ptr = a_objects.at(i);a_objects.erase(a_objects.begin() + i);break;}}
        if(object_shared_ptr.get() != 0){a_objects.insert(a_objects.begin(), object_shared_ptr);}
    }
    ...
    scls::Fraction Graphic::left_x() const {return scls::Fraction::from_double(a_graphic_base.get()->x_middle_in_canonical_base()) - width() / 2;}
    scls::Point_2D Graphic::middle() const {return scls::Point_2D(a_graphic_base.get()->x_middle_in_canonical_base(), a_graphic_base.get()->y_middle_in_canonical_base());}
    double Graphic::middle_x() const {return a_graphic_base.get()->a_x_middle;}
    void Graphic::middle_x_add(double value) {a_graphic_base.get()->set_x_middle_in_canonical_base(a_graphic_base.get()->x_middle_in_canonical_base() + value);}
    double Graphic::middle_y() const {return a_graphic_base.get()->a_y_middle;}
    void Graphic::middle_y_add(double value) {a_graphic_base.get()->set_y_middle_in_canonical_base(a_graphic_base.get()->y_middle_in_canonical_base() + value);}
    void Graphic::set_middle(scls::Point_2D new_middle){a_graphic_base.get()->set_x_middle_in_canonical_base(new_middle.x());a_graphic_base.get()->set_y_middle_in_canonical_base(new_middle.y());}

    // Update the base
    void Graphic::update_base() {
        a_graphic_base.get()->set_height_unit_in_canonical_base(a_graphic_base.get()->a_height_in_pixel / a_graphic_base.get()->a_height);
        a_graphic_base.get()->set_width_unit_in_canonical_base(a_graphic_base.get()->a_width_in_pixel / a_graphic_base.get()->a_width);
        a_graphic_base.get()->set_x_middle_in_base(a_graphic_base.get()->a_width / 2.0 + a_graphic_base.get()->a_x_middle);
        a_graphic_base.get()->set_y_middle_in_base(a_graphic_base.get()->a_height / 2.0 + a_graphic_base.get()->a_y_middle);
    }

    // Struct containing some datas for positions
    //struct Needed_Pos {scls::__Formula pos;scls::__Formula previous_pos;bool previous_pos_used = false;};
    // Draw the bases of the image
    void __image_draw_base(std::shared_ptr<scls::__Image_Base> image, Plane_Base* plane) {
        scls::draw_grid(image, plane);
        return;

        // Calculations
        plane->a_pixel_by_case_x = static_cast<double>(image.get()->width()) / plane->a_width;
        plane->a_pixel_by_case_y = static_cast<double>(image.get()->height()) / plane->a_height;

        // Horizontal line
        double current_y = plane->pixel_y_to_graphic_y(image.get()->height(), image).to_double();
        current_y = floor(current_y);
        double needed_y = plane->graphic_y_to_pixel_y(current_y, image);
        while(needed_y < image.get()->width()) {
            ...
            current_y++;
            needed_y = plane->graphic_y_to_pixel_y(current_y, image);
        }

        // Vertical lines
        double current_x = plane->pixel_x_to_graphic_x(0, image).to_double();
        current_x = ceil(current_x);
        int needed_x = plane->graphic_x_to_pixel_x(current_x, image);
        while(needed_x < image.get()->width()) {
            ...
            current_x++;
            needed_x = plane->graphic_x_to_pixel_x(current_x, image);
        }
    }
    void Graphic::image_draw_base(std::shared_ptr<scls::__Image_Base> image) {
        scls::draw_grid(image, a_graphic_base.get());
        return;

        // Horizontal lines
        double current_y = pixel_y_to_graphic_y(0, image).to_double();
        current_y = floor(current_y);
    ...
    void Graphic::image_draw_function(std::shared_ptr<scls::__Image_Base> to_return, std::shared_ptr<Graphic_Function> needed_function, std::vector<scls::Fraction>& screen_pos) {
        ...
        std::vector<std::string> unknowns = needed_function.get()->formula().all_unknowns_name();
        if(unknowns.size() == 1) {
            scls::draw_function_graph(to_return, &needed_function.get()->formula(), graphic_base());
			...
			for(int i = 0;i<static_cast<int>(to_return.get()->width()) + 1;i++){
				scls::Fraction value = needed_pos[i].pos.value_to_fraction().normalized();
				needed_y[i].pos = graphic_y_to_pixel_y(value.to_double(), to_return);
				// Check the previous pos
				if(needed_pos[i].previous_pos_used) {
					value = needed_pos[i].previous_pos.value_to_fraction().normalized();
        ...
        else if(unknowns.size() == 2) {
        	// Draw each pixel
			/*for(int j = 0;j<static_cast<int>(to_return.get()->width());j++) {
				for(int k = 0;k<static_cast<int>(to_return.get()->width());k++) {
					scls::Fraction x = pixel_x_to_graphic_x(j, to_return.get()->width());scls::Fraction y = pixel_y_to_graphic_y(to_return.get()->height() - k, to_return.get()->height());

					scls::__Formula_Base::Unknowns_Container u_c;
					scls::__Formula_Base::Unknown* u = u_c.create_unknown("x");
					u->set_value(std::make_shared<scls::__Formula>(x));
					u = u_c.create_unknown("y");
					u->set_value(std::make_shared<scls::__Formula>(y));
					double value = needed_function.get()->formula().value(&u_c).to_double() / 20.0;
					if(value != 0){value = std::log(std::abs(value)) * scls::sign(value);}
					/*if(value == 0){value = 1;}
					else{value = 1.0/value;}
					if(value > 1.0){value=1.0/value;}
					value = std::pow(value, 10) * scls::sign(value);

					if(value < 0) {to_return.get()->set_pixel(j, k, scls::Color(0, 0, 255.0 * -value));}
					else{to_return.get()->set_pixel(j, k, scls::Color(255.0 * value, 0, 0));}
				}
			}//*/
        }
    }
    ...
    scls::Fraction Graphic::pixel_x_to_graphic_x(int x, int image_width){return a_graphic_base.get()->pixel_x_to_graphic_x(x, image_width);}
    scls::Fraction Graphic::pixel_x_to_graphic_x(int x, std::shared_ptr<scls::__Image_Base> needed_image){return a_graphic_base.get()->pixel_x_to_graphic_x(x, needed_image);}
    scls::Fraction Graphic::pixel_y_to_graphic_y(int y, int needed_height){return a_graphic_base.get()->pixel_y_to_graphic_y(y, needed_height);}
    scls::Fraction Graphic::pixel_y_to_graphic_y(int y, std::shared_ptr<scls::__Image_Base> needed_image){return a_graphic_base.get()->pixel_y_to_graphic_y(y, needed_image);}
    ...
    bool Graphic::__update_action(__Graphic_Object_Base* object, scls::Action_Structure* structure, double used_delta_time){
        if(structure->next_action() != 0){
            ...
            if(structure->next_action_type() == ACTION_ACCELERATE) {
                ...
                // Call the function
                __Graphic_Object_Base::Action_Function* needed_function = l_a->needed_function.get();
                if(needed_function == 0){
                    l_a->needed_function = action_function_by_name(l_a->function_name)->clone_as_function();needed_function = l_a->needed_function.get();
                    if(needed_function == 0){scls::print("PLEOS Graphic \"actions\"", std::string("The action function \"") + l_a->function_name + std::string("\" does not exist."));action_terminated=true;}
                    else{
                        // Start the normal update of the action
                        needed_function->go_to_first_action();
                        if(l_a->function_namespace.get() != 0){environment()->add_namespace_stack(l_a->function_namespace);}
                        __update_action(object, needed_function, used_delta_time);
                        if(l_a->function_namespace.get() != 0){environment()->pop_namespace_stack();}
                        if(needed_function->is_end_action()){action_terminated = true;}
                    }
                }
                else{
                    // Normally update the action
                    if(l_a->function_namespace.get() != 0){environment()->add_namespace_stack(l_a->function_namespace);}
                    __update_action(object, needed_function, used_delta_time);
                    if(l_a->function_namespace.get() != 0){environment()->pop_namespace_stack();}
                    if(needed_function->is_end_action()){action_terminated = true;}
                ...
                // Do the movement
                double distance = l_a->position_end().distance(object->position());
                if(direction_vector.norm() < distance){object->move_x(direction_vector.x());object->move_y(direction_vector.y());}//object->move_x(direction_vector.x());object->move_y(direction_vector.y());
    ...
    void Graphic::update_delete(){
        ...
        for(int i = 0;i<static_cast<int>(physic_objects().size());i++){if(physic_object(i)->attached_object() == 0 || physic_object(i)->attached_object()->should_delete()){physic_object(i)->delete_object();}}
    }
    void Graphic::update(double used_delta_time){
        ...
        if(a_actions.get()->next_action() != 0){
            ...
            // If the action is terminated
            if(action_terminated){a_actions.get()->go_to_next_action();}
        }
    ...
    bool Graphic::graphic_from_xml_balise_attribute_circle(scls::XML_Attribute& attribute, std::shared_ptr<Circle> circle, Text_Environment* environment, scls::Text_Style text_style) {
        ...
        if(attribute.name == "angle_end") {circle.get()->set_angle_end(environment->value_double(attribute.value));}
        else if(attribute.name == "angle_start") {circle.get()->set_angle_start(environment->value_double(attribute.value));}
        ...
        else if(attribute.name == "radius") {double f = environment->value_double(attribute.value);circle.get()->set_radius_x(f);circle.get()->set_radius_y(f);}
        else if(attribute.name == "radius_x") {circle.get()->set_radius_x(environment->value_double(attribute.value));}
        else if(attribute.name == "radius_y") {circle.get()->set_radius_y(environment->value_double(attribute.value));}
    ...
    bool Graphic::graphic_from_xml_balise_attribute_form_2d(scls::XML_Attribute& attribute, std::shared_ptr<Form_2D> created_form, Text_Environment* environment, scls::Text_Style text_style){
        if(attribute.name == "border_color") {created_form.get()->set_border_color(environment->value_color(attribute.value));return true;}
        ...
        else if(attribute.name == "color") {created_form.get()->set_color(environment->value_color(attribute.value));return true;}
            ...
            if(created_form.get()->points().size() <= 0){
                std::shared_ptr<Point_2D> created_point = created_form.get()->new_point(environment->value_double(attribute.value), 0);
                add_point(created_point);set_object_render_before(created_point, created_form.get());
            }
            else{created_form.get()->points().at(0).get()->set_x(environment->value_double(attribute.value));}
        }
            ...
            else{created_form.get()->points().at(1).get()->set_x(environment->value_double(attribute.value));}
        }
            ...
            else{created_form.get()->points().at(2).get()->set_x(environment->value_double(attribute.value));}
        }
            ...
            else{created_form.get()->points().at(3).get()->set_x(environment->value_double(attribute.value));}
        }
        else if(attribute.name == "y_1") {
            if(created_form.get()->points().size() <= 0){
                std::shared_ptr<Point_2D> created_point = created_form.get()->new_point(0, environment->value_double(attribute.value));
                add_point(created_point);set_object_render_before(created_point, created_form.get());
            }
            else{created_form.get()->points().at(0).get()->set_y(environment->value_double(attribute.value));}
        }
            ...
            else{created_form.get()->points().at(1).get()->set_y(environment->value_double(attribute.value));}
        }
            ...
            else{created_form.get()->points().at(2).get()->set_y(environment->value_double(attribute.value));}
        }
            ...
            else{created_form.get()->points().at(3).get()->set_y(environment->value_double(attribute.value));}
        }
    ...
    bool Graphic::graphic_from_xml_balise_attribute_physic(scls::XML_Attribute& attribute, std::shared_ptr<__Graphic_Object_Base> object, std::shared_ptr<Graphic_Physic>& physic, Text_Environment* environment, scls::Text_Style text_style) {
        ...
        if(attribute.name == "collision") {if(attribute.value == std::string("circle")){physic.get()->new_collision(scls::Collision_Type::GCT_Circle);}else if(attribute.value == std::string_view("mesh")){reinterpret_cast<Form_2D*>(object.get())->add_collision_mesh(physic.get());}else if(attribute.value == std::string("rect")){physic.get()->new_collision(scls::Collision_Type::GCT_Rect);}else{physic.get()->new_collision(scls::Collision_Type::GCT_Line);}}
        else if(attribute.name == "gravity" || attribute.name == "use_gravity") {physic.get()->set_use_gravity((attribute.value == std::string("1") || attribute.value == std::string_view("true")));}
        else if(attribute.name == "ignore_dynamic_collisions" ||attribute.name == "ignore_dynamic_collision") {physic.get()->set_ignore_dynamic_collisions((attribute.value == std::string_view("1") || attribute.value == std::string("true")));}
    ...
    void Graphic::__new_line_xml(std::shared_ptr<pleos::__Graphic_Object_Base>& o_1, std::shared_ptr<pleos::__Graphic_Object_Base>& o_2, double& x_1, double& x_2, double& y_1, double& y_2, std::shared_ptr<scls::XML_Text_Base> xml, Text_Environment* environment) {
        std::vector<scls::XML_Attribute>& attributes = xml.get()->xml_balise_attributes();
        for(int j = 0;j<static_cast<int>(attributes.size());j++) {
            if(attributes[j].name == "x_start" || attributes[j].name == "x_1") {x_1 = environment->value_double(attributes[j].value);}
            else if(attributes[j].name == "x_end" || attributes[j].name == "x_2") {x_2 = environment->value_double(attributes[j].value);}
            else if(attributes[j].name == "y_start" || attributes[j].name == "y_1") {y_1 = environment->value_double(attributes[j].value);}
            else if(attributes[j].name == "y_end" || attributes[j].name == "y_2") {y_2 = environment->value_double(attributes[j].value);}
            else if(attributes[j].name == std::string_view("points")) {
                std::vector<std::string> cut = scls::cut_string(attributes[j].value, std::string(";"));
                o_1 = object_by_name_shared_ptr(cut.at(0));
                o_2 = object_by_name_shared_ptr(cut.at(1));
            }
        }
    }
    ...
    std::shared_ptr<pleos::__Graphic_Object_Base> Graphic::graphic_from_xml_balise(std::shared_ptr<scls::XML_Text_Base> xml, Text_Environment* environment, scls::Text_Style text_style){
        ...
        if(current_balise_name == std::string_view("application")) {
            std::string expression = std::string("x");std::string images_direct = std::string();
            std::string from = std::string();std::string to = std::string();
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                if(attributes.at(j).name == std::string_view("expression")){expression = attributes.at(j).value;}
                else if(attributes[j].name == "from") {from = attributes[j].value;}
                else if(attributes[j].name == "images_direct") {images_direct = attributes[j].value;}
                else if(attributes[j].name == "to") {to = attributes[j].value;}
            }
            std::shared_ptr<scls::Formula_Base> f = scls::string_to_algebra_element<scls::Formula_Base>(expression);

            // Get the datas
            Set* set_end = reinterpret_cast<Set*>(object_by_name(to));
            Set* set_start = reinterpret_cast<Set*>(object_by_name(from));
            if(set_end == 0){scls::print("PLEOS Graphic", std::string("The object \"") + to + std::string("\" you want to use as the end of an application does not exist."));}
            else if(set_start == 0){scls::print("PLEOS Graphic", std::string("The object \"") + to + std::string("\" you want to use as the start of an application does not exist."));}
            else {
                std::string name = set_start->name() + std::string("-") + set_end->name();

                // Get the images
                int card = set_start->cardinal();
                std::vector<std::string> cut = scls::cut_string(images_direct, std::string(";"));
                std::vector<int> images = std::vector<int>(card, -1);
                for(std::size_t i = 0;i<cut.size();i++){std::vector<std::string> cut_2 = scls::cut_string(cut.at(i), std::string("-"));int a = std::stoi(cut_2.at(0));int b = std::stoi(cut_2.at(1));images[a] = b;}
                for(int i = 0;i<card;i++){if(images.at(i) == -1){images[i] = (f.get()->replace_unknowns(std::string("x"), i + 1).get()->value<scls::Fraction>()->to_double()) - 1;}}

                // Create the application
                double offset_x = 0.5;
                for(int i = 0;i<card;i++) {
                    Graphic::Graphic_Text* first = set_start->element(i);
                    Graphic::Graphic_Text* last = set_end->element(images.at(i));
                    std::shared_ptr<Form_2D> current_line = new_line(name + std::string("-") + std::to_string(i), 0, first->absolute_x() + offset_x, first->absolute_y(), last->absolute_x() - offset_x, last->absolute_y());
                    current_line.get()->set_border_color(scls::Color(0, 0, 0));current_line.get()->set_border_width(3);
                }
            }
        }
            ...
            // Add the form
            std::shared_ptr<Arrow_2D> created_form = new_line_xml<Arrow_2D>(xml, environment);
            ...
            // Add the hat
            std::shared_ptr<Circle> p_1 = new_circle<Circle>(needed_name + std::string("-p_1"), 0, 0.25, 0.25, 0.25, 0.25);p_1.get()->set_opacity(0);
            std::shared_ptr<Circle> p_2 = new_circle<Circle>(needed_name + std::string("-p_2"), 0, 0.25, -0.25, 0.25, 0.25);p_2.get()->set_opacity(0);
            std::shared_ptr<Form_2D> hat_1 = new_line(needed_name + std::string("-hat_1"), p_1, created_form.get()->points().at(1));
            std::shared_ptr<Form_2D> hat_2 = new_line(needed_name + std::string("-hat_2"), p_2, created_form.get()->points().at(1));
            hat_1.get()->set_border_color(border_color);hat_1.get()->set_save_to_xml_text(false);
            hat_2.get()->set_border_color(border_color);hat_2.get()->set_save_to_xml_text(false);
            hat_1.get()->set_border_radius(border_radius.to_double());
            ...
            double needed_height = height();double needed_width = width();
            bool heigth_used = false;bool width_used = false;
            if(needed_height == -1){needed_height = 10;}if(needed_width == -1){needed_width = 10;}
            double new_middle_x = middle_x();double new_middle_y = middle_y();
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                if(attributes[j].name == "height") {needed_height = environment->value_double(attributes[j].value);heigth_used=true;}
                else if(attributes[j].name == "width") {needed_width = environment->value_double(attributes[j].value);width_used=true;}
                else if(attributes[j].name == "x") {new_middle_x = environment->value_double(attributes[j].value);}
                else if(attributes[j].name == "y") {new_middle_y = environment->value_double(attributes[j].value);}
            ...
            set_middle(new_middle_x, new_middle_y);set_scale(needed_width, needed_height, width_used, heigth_used);
        }
            ...
            std::vector<std::shared_ptr<scls::XML_Text_Base>>& sub_texts = xml.get()->sub_texts();
            ...
            // Add the function
            needed_function.get()->set_formula_shared_ptr(environment->value_formula(needed_expression));
            ...
            created_datas_set.get()->set_height(needed_height.to_double());created_datas_set.get()->set_width(needed_width.to_double());
            created_datas_set.get()->set_x(needed_x.to_double());created_datas_set.get()->set_y(needed_y.to_double());
            ...
            // Add the form
            std::shared_ptr<Form_2D> created_form = new_line_xml<Form_2D>(xml, environment);
            created_form.get()->set_border_color(border_color);created_form.get()->set_border_radius(border_radius.to_double());
            created_form.get()->set_object_name(std::string("line"));created_form.get()->link(0).drawing_proportion = needed_proportion;
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {if(graphic_from_xml_balise_attribute_object(attributes.at(j), created_form, environment, text_style)) {}}
            ...
            if(use_physic != 0){
                ...
                if(use_collision){
                    ...
                    double x_2 = created_form.get()->points().at(1).get()->absolute_x();
                    double y_1 = created_form.get()->points().at(0).get()->absolute_y();
                    double y_2 = created_form.get()->points().at(1).get()->absolute_y();
                    physic.get()->add_collision(x_1, y_1, x_2, y_2);
                }
                physic.get()->set_save_to_xml_text(false);
            }
        }
        else if(current_balise_name == "plane") {
            // Add the plane
            std::shared_ptr<Graphic::Graphic_Texture_Plane> new_object = new_texture_object<Graphic::Graphic_Texture_Plane>(std::string(""), 0, 0);
            object = new_object;
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                if(attributes.at(j).name == std::string("plane_height")){new_object.get()->plane()->a_height = environment->value_double(attributes.at(j).value);}
                else if(attributes.at(j).name == std::string("plane_width")){new_object.get()->plane()->a_width = environment->value_double(attributes.at(j).value);}
                else if(graphic_from_xml_balise_attribute_texture_object(attributes[j], new_object, environment, text_style)){}
            }

            // Set the texture
            new_object.get()->set_texture(scls::Image(500, 500, scls::Color(255, 255, 255)));
            __image_draw_base(new_object.get()->texture_shared_ptr(), new_object.get()->plane());
        }
            ...
            int use_physic = 1; // 0 = None, 1 = Static, 2 = Dynamic
            ...
            // Get the datas about a vector of the graphic
            std::string needed_name = std::string();double needed_x = 0;double needed_y = 0;
            ...
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                ...
                else if(attributes[j].name == "x") {needed_x = environment->value_double(attributes[j].value);}
                else if(attributes[j].name == "y") {needed_y = environment->value_double(attributes[j].value);}
            }
            ...
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                if(attributes[j].name == "length") {created_object.get()->set_width(environment->value_double(attributes[j].value));}
                else if(graphic_from_xml_balise_attribute_object(attributes.at(j), created_object, environment, text_style)){}
            }

            // Create the line
            int number_end = 2;int number_start = -2;
            number_line(created_object.get()->x(), created_object.get()->y(), created_object.get()->width(), number_start, number_end);
        }
            ...
            // Add the form
            object = created_form;
            created_form.get()->set_height(height.to_double());
            created_form.get()->set_object_name(std::string("rect"));
            created_form.get()->set_width(width.to_double());
            created_form.get()->set_x(created_form.get()->x() + width.to_double() / 2);
            ...
            enum Needed_Object {NO_Circle, NO_Rect};
            ...
            scls::Fraction needed_max_radius = 1;scls::Fraction needed_min_radius = 1;
            scls::Fraction needed_max_scale_x = 1;scls::Fraction needed_max_scale_y = 1;
            scls::Fraction needed_min_scale_x = 1;scls::Fraction needed_min_scale_y = 1;
            ...
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                ...
                else if(attributes[j].name == "max_scale_y") {needed_max_scale_y = scls::Fraction::from_std_string(attributes[j].value);}
                else if(attributes[j].name == "min_radius") {needed_min_radius = scls::Fraction::from_std_string(attributes[j].value);}
                else if(attributes[j].name == "min_scale_x") {needed_min_scale_x = scls::Fraction::from_std_string(attributes[j].value);}
                else if(attributes[j].name == "min_scale_y") {needed_min_scale_y = scls::Fraction::from_std_string(attributes[j].value);}
                ...
                else if(attributes[j].name == "opacity") {needed_opacity = environment->value_number(attributes[j].value).to_double();}
            }

            // Add the objects
            if(needed_object == Needed_Object::NO_Circle) {
                for(int j = 0;j<number;j++) {
                    scls::Fraction random_radius = scls::random_fraction(needed_min_radius, needed_max_radius);
                    ...
                    std::shared_ptr<Circle> created_circle = new_circle<Circle>(needed_name + std::to_string(objects().size()), random_x.to_double(), random_y.to_double(), needed_radius.to_double());
                    ...
                    created_circle.get()->set_opacity(needed_opacity);
                    created_circle.get()->set_radius_x(random_radius.to_double());
                    created_circle.get()->set_radius_y(random_radius.to_double());
                }
            }
        }
        else if(current_balise_name == "set") {
            // Get the datas about a text of the graphic
            scls::Text_Style current_style = text_style.new_child();
            std::shared_ptr<Set> parent_set = new_object<Set>(std::string());
            int needed_cardinal = 5;
            std::string needed_content = std::string();std::string needed_name = std::string();
            scls::Fraction needed_font_size_in_scale = -1;
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                if(attributes[j].name == "cardinal") {needed_cardinal = environment->value_double(attributes[j].value);}
                ...
                else if(graphic_from_xml_balise_attribute_object(attributes.at(j), parent_set, environment, current_style)){}
            }

            // Add the text
            double element_height = 2;
            double y_start = -(static_cast<double>(needed_cardinal - 1) / 2.0) * element_height;
            double current_y = y_start;
            for(int i = 0;i<needed_cardinal;i++) {
                std::shared_ptr<Graphic::Graphic_Text> created_text = new_text(parent_set.get()->name() + std::string("_") + std::to_string(i), std::to_string(i + 1), 0, current_y, current_style);
                created_text.get()->set_parent(parent_set);parent_set.get()->add_element(created_text);
            ...
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                ...
                else if(scls::text_style_from_xml_attribute(&attributes.at(j), current_style)){}
                else if(graphic_from_xml_balise_attribute_object(attributes.at(j), created_text, environment, current_style)){}
            }
            ...
            // Add the variable
            unknowns()->create_unknown(needed_name)->value = environment->value_formula(needed_value);
        }
    ...
    std::shared_ptr<pleos::__Graphic_Object_Base> Graphic::graphic_from_xml_balise_action(std::shared_ptr<scls::XML_Text_Base> xml, Text_Environment* environment, scls::Text_Style text_style, scls::Action_Structure* structure) {
        ...
        std::string current_balise_name = xml.get()->xml_balise_name();
        std::vector<scls::XML_Attribute>& attributes = xml.get()->xml_balise_attributes();
        std::vector<std::shared_ptr<pleos::__Graphic_Object_Base>> needed_objects;

        // Get the pre-needed datas
        #define LOAD_PRENEEDED_DATAS\
        std::string wait_time = std::string();double wait_until = -1;\
        for(int i = 0;i<static_cast<int>(attributes.size());i++){\
            if(attributes.at(i).name == "name" || attributes.at(i).name == "object") {\
                std::shared_ptr<__Graphic_Object_Base> needed_object = object_by_name_shared_ptr(attributes[i].value);\
                if(needed_object.get()==0){scls::print(std::string("PLEOS Graphic"), std::string("The object \"") + attributes[i].value + std::string("\" you want to add an action does not exists."));}\
        ...
        if(wait_time != std::string()){\
            if(structure != 0){structure->add_action(std::make_shared<__Graphic_Object_Base::Action_Wait>(environment->value_double(wait_time))).get()->save_to_xml_text = true;}\
                ...
                for(int i = 0;i<static_cast<int>(needed_objects.size());i++){\
                    environment->set_unknown_value_by_name("x", i);\
                    needed_objects.at(i).get()->actions()->add_action(std::make_shared<__Graphic_Object_Base::Action_Wait>(environment->value_double(wait_time))).get()->save_to_xml_text = true;\
                }\
            }\
        }\
        if(wait_until != -1){\
            if(structure != 0){structure->add_action(std::make_shared<__Graphic_Object_Base::Action_Wait_Until>(wait_until)).get()->save_to_xml_text = true;}\
                ...
                for(int i = 0;i<static_cast<int>(needed_objects.size());i++){\
                    needed_objects.at(i).get()->actions()->add_action(std::make_shared<__Graphic_Object_Base::Action_Wait_Until>(wait_until)).get()->save_to_xml_text = true;\
                }\
            }\
        }
        ...
        if(current_balise_name == std::string("action")) {
                    ...
                    for(int i = 2;i<max_n_needed;i++) {
                        if(pleos::number(i)->is_prime()) {
                            #define SET_CASE_COLOR(x, y, color, time, wait) if(structure != 0){structure->add_action(std::make_shared<__Graphic_Object_Base::Action_Wait>(wait)).get()->save_to_xml_text=true;structure->add_action(std::make_shared<__Graphic_Object_Base::Action_Set_Parameter>(std::string("case_background_color-") + std::to_string(x) + std::string("-") + std::to_string(y), color.to_std_string(0), time)).get()->save_to_xml_text = true;}\
                                ...
                                for(int i = 0;i<static_cast<int>(needed_objects.size());i++) {\
                                    needed_objects.at(i).get()->actions()->add_action(std::make_shared<__Graphic_Object_Base::Action_Wait>(wait)).get()->save_to_xml_text=true;needed_objects.at(i).get()->actions()->add_action(std::make_shared<__Graphic_Object_Base::Action_Set_Parameter>(std::string("case_background_color-") + std::to_string(x) + std::string("-") + std::to_string(y), color.to_std_string(0), time)).get()->save_to_xml_text = true;\
                                }\
                            }
            ...
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                if(attributes[j].name == "function") {function_name = attributes[j].value;}
            }

            // Add the action
            if(structure != 0){structure->add_action(std::make_shared<__Graphic_Object_Base::Action_Function_Call>(function_name));structure->last_action()->save_to_xml_text = true;}
            else if(needed_objects.size() > 0){
                for(int i = 0;i<static_cast<int>(needed_objects.size());i++) {
                    needed_objects.at(i).get()->actions()->add_action(std::make_shared<__Graphic_Object_Base::Action_Function_Call>(function_name));needed_objects.at(i).get()->last_action()->save_to_xml_text = true;
                }
            }
            else{scls::print("PLEOS Graphic \"actions\"", std::string("An action of type \"") + current_balise_name + std::string("\" has no object specified."));}
        }
            ...
            // Get the datas about a vector of the graphic
            LOAD_PRENEEDED_DATAS
            double needed_speed = 1;
            double needed_x = 0;bool x_used = false;
            double needed_y = 0;bool y_used = false;
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                if(attributes[j].name == "speed") {needed_speed = environment->value_double(attributes[j].value);}
                else if(attributes[j].name == "x") {needed_x = environment->value_double(attributes[j].value);x_used = true;}
                else if(attributes[j].name == "y") {needed_y = environment->value_double(attributes[j].value);y_used = true;}
            }

            // Add the action
            if(structure != 0){structure->add_action(std::make_shared<__Graphic_Object_Base::Action_Move>(needed_x, needed_y, needed_speed)).get()->save_to_xml_text = true;}
            else if(needed_objects.size() > 0){
                for(int i = 0;i<static_cast<int>(needed_objects.size());i++) {
                    needed_objects.at(i).get()->actions()->add_action(std::make_shared<__Graphic_Object_Base::Action_Move>(needed_x, needed_y, needed_speed)).get()->save_to_xml_text = true;
                }
            }
        }
            ...
            LOAD_PRENEEDED_DATAS
            double needed_speed = 1;
            double needed_rotation = 0;
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                if(attributes[j].name == "speed") {needed_speed = environment->value_double(attributes[j].value);}
                else if(attributes[j].name == "rotation" || attributes[j].name == std::string_view("value")) {needed_rotation = environment->value_double(attributes[j].value);}
            }

            // Add the action
            if(structure != 0){structure->add_action(std::make_shared<__Graphic_Object_Base::Action_Rotate>(needed_rotation, needed_speed)).get()->save_to_xml_text = true;}
            else if(needed_objects.size() > 0){
                for(int i = 0;i<static_cast<int>(needed_objects.size());i++) {
                    needed_objects.at(i).get()->actions()->add_action(std::make_shared<__Graphic_Object_Base::Action_Rotate>(needed_rotation, needed_speed)).get()->save_to_xml_text = true;
                }
            }
        }
            ...
            std::string needed_parameter_value = std::string("0");
            double needed_time = 0;
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {if(attributes[j].name == "time") {needed_time = environment->value_double(attributes[j].value);}}

            // Add the action
            if(structure != 0){structure->add_action(std::make_shared<__Graphic_Object_Base::Action_Stop>(needed_time)).get()->save_to_xml_text = true;}
            else if(needed_objects.size() > 0){
                for(int i = 0;i<static_cast<int>(needed_objects.size());i++) {
                    needed_objects.at(i).get()->actions()->add_action(std::make_shared<__Graphic_Object_Base::Action_Stop>(needed_time)).get()->save_to_xml_text = true;
                }
            }
        }
            ...
            double needed_time = 0;
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                if(attributes[j].name == "duration" || attributes[j].name == "time") {needed_time = environment->value_double(attributes[j].value);}
            }

            // Add the action
            if(structure != 0){structure->add_action(std::make_shared<__Graphic_Object_Base::Action_Wait>(needed_time));structure->last_action()->save_to_xml_text = true;}
            else if(needed_objects.size() > 0){
                for(int i = 0;i<static_cast<int>(needed_objects.size());i++) {
                    needed_objects.at(i).get()->actions()->add_action(std::make_shared<__Graphic_Object_Base::Action_Wait>(needed_time));needed_objects.at(i).get()->last_action()->save_to_xml_text = true;
                }
            }
            else{scls::print("PLEOS Graphic \"actions\"", std::string("An action of type \"") + current_balise_name + std::string("\" has no object specified."));}
        }
            ...
            // Add the action
            if(structure != 0){structure->add_action(std::make_shared<__Graphic_Object_Base::Action_Wait_Until>(needed_time));structure->last_action()->save_to_xml_text = true;}
            else if(needed_objects.size() > 0){
                for(int i = 0;i<static_cast<int>(needed_objects.size());i++) {
                    needed_objects.at(i).get()->actions()->add_action(std::make_shared<__Graphic_Object_Base::Action_Wait_Until>(needed_time));needed_objects.at(i).get()->last_action()->save_to_xml_text = true;
                }
            }
        }

        // Structures
        else if(current_balise_name == std::string("action_function") || current_balise_name == std::string("function_action") || current_balise_name == std::string("function")) {
            // Get the datas about the wait action
            std::string function_name = std::string();
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                if(attributes[j].name == "name") {function_name = attributes[j].value;}
            }

            // Get the content
            std::shared_ptr<__Graphic_Object_Base::Action_Function> new_structure = std::make_shared<__Graphic_Object_Base::Action_Function>(function_name);
            ...
            // Create the function
            a_actions_functions.push_back(new_structure);
        }
        else if(current_balise_name == std::string("action_if") || current_balise_name == std::string("if_action")) {
            // Get the datas about the wait action
            LOAD_PRENEEDED_DATAS
            /*int repetitions = 0;
            for(int j = 0;j<static_cast<int>(attributes.size());j++) {
                if(attributes[j].name == "repetition" || attributes[j].name == "repetitions") {repetitions = environment->value_double(attributes[j].value);}
            }//*/

            // Get the content
            std::shared_ptr<__Graphic_Object_Base::Action_If> new_structure = std::make_shared<__Graphic_Object_Base::Action_If>();
            for(int i = 0;i<static_cast<int>(xml.get()->sub_texts().size());i++){
                graphic_from_xml_balise_action(xml.get()->sub_texts().at(i), environment, text_style, new_structure.get());
            }
            ...
            if(structure != 0){structure->add_action(new_structure);structure->last_action()->save_to_xml_text = true;}
                ...
                for(int i = 0;i<static_cast<int>(needed_objects.size());i++) {
                    needed_objects.at(i).get()->actions()->add_action(new_structure).get()->save_to_xml_text = true;
                }
            }
        }
        ...
        if(needed_objects.size() == 0){return std::shared_ptr<pleos::__Graphic_Object_Base>();}
        return needed_objects.at(0);
    }
    void Graphic::__graphic_from_xml_balises(std::shared_ptr<scls::XML_Text_Base> xml, Text_Environment* environment, scls::Text_Style text_style, int graphic_width_in_pixel, int graphic_height_in_pixel){
    	...
    	if(xml.get()->xml_balise_name() != std::string_view("graphic") && xml.get()->xml_balise_name() != std::string_view("")) {
            ...
            for(int i = 0;i<static_cast<int>(xml->sub_texts().size());i++) {
            	__graphic_from_xml_balises(xml->sub_texts().at(i), environment, text_style, graphic_width_in_pixel, graphic_height_in_pixel);
            }
	    }
	}
    ...
    void Graphic::graphic_from_xml(std::shared_ptr<scls::XML_Text_Base> xml, scls::Text_Style needed_style, std::shared_ptr<Text_Environment> environment, int* graphic_width_in_pixel, int* graphic_height_in_pixel) {
        ...
        double graphic_width = 10;
        double graphic_height = graphic_width * scls::Fraction(*graphic_height_in_pixel, *graphic_width_in_pixel).to_double();
        double graphic_x = 0;double graphic_y = 0;
        ...
        if(graphic_width != 0 && graphic_height == 0){graphic_height = graphic_width * scls::Fraction(*graphic_height_in_pixel, *graphic_width_in_pixel).to_double();}
        else if(graphic_width == 0 && graphic_height != 0){graphic_width = graphic_height * scls::Fraction(*graphic_width_in_pixel, *graphic_height_in_pixel).to_double();}
        else if(graphic_width == 0 && graphic_height == 0){graphic_width = 10;graphic_height = graphic_width * scls::Fraction(*graphic_height_in_pixel, *graphic_width_in_pixel).to_double();}
        set_middle(graphic_x, graphic_y);
        set_scale(graphic_width, graphic_height, width_used(), height_used());

        // Get the image
        if(use_ratio_base){
            if(graphic_height_in_pixel_used&&!graphic_width_in_pixel_used){(*graphic_width_in_pixel) = static_cast<double>(*graphic_height_in_pixel) * (graphic_width / graphic_height);}
            else if(!graphic_height_in_pixel_used&&graphic_width_in_pixel_used){(*graphic_height_in_pixel) = static_cast<double>(*graphic_width_in_pixel) * (graphic_height / graphic_width);}
            else if(!graphic_height_in_pixel_used&&!graphic_width_in_pixel_used){
                (*graphic_width_in_pixel) = static_cast<double>(*graphic_height_in_pixel) * (graphic_width / graphic_height);
            }
        }
    ...
    void Graphic::graphic_from_xml(std::string to_parse, int graphic_width_in_pixel, int graphic_height_in_pixel) {graphic_from_xml(scls::xml(balises_shared_ptr(), to_parse), scls::Text_Style(), &graphic_width_in_pixel, &graphic_height_in_pixel);}
    ...
    std::vector<Graphic::Graphic_Physic*> Graphic::physic_objects_in_rect(double x, double y, double width, double height) {
        std::vector<std::shared_ptr<scls::Physic_Object>> start = a_physic_engine.physic_objects_in_rect(x, y, width, height);
        std::vector<Graphic::Graphic_Physic*> to_return = std::vector<Graphic::Graphic_Physic*>(start.size());
        for(int i = 0;i<static_cast<int>(start.size());i++){to_return[i] = reinterpret_cast<Graphic::Graphic_Physic*>(start.at(i).get());}return to_return;
    }

    // Raycasts the map
    scls::Physic_Engine::Raycast_Result Graphic::raycast(double x_start, double y_start, double x_direction, double y_direction, double distance){return a_physic_engine.raycast(x_start, y_start, x_direction, y_direction, distance);}
    ...
    __Graphic_Object_Base* Graphic::object_by_physic_object(scls::Physic_Object* attached_object){return object_by_physic_object_shared_ptr(attached_object).get();};
    std::shared_ptr<__Graphic_Object_Base> Graphic::object_by_physic_object_shared_ptr(scls::Physic_Object* attached_object) {return reinterpret_cast<Graphic::Graphic_Physic*>(attached_object)->attached_object_shared_ptr();}
    Graphic::Graphic_Physic* Graphic::physic_object_by_attached_object(__Graphic_Object_Base* attached_object){return reinterpret_cast<Graphic::Graphic_Physic*>(physic_object_by_attached_object_shared_ptr(attached_object).get());}
    std::shared_ptr<scls::Physic_Object> Graphic::physic_object_by_attached_object_shared_ptr(__Graphic_Object_Base* attached_object) {
        // Asserts
        if(attached_object == 0){return std::shared_ptr<Graphic_Physic>();}

        for(int i = 0;i<static_cast<int>(physic_objects().size());i++){if(physic_object(i)->attached_object() == attached_object){return physic_objects().at(i);}}
        return std::shared_ptr<scls::Physic_Object>();
    }
    ...
    void Graphic::number_line(scls::Fraction x, scls::Fraction y, scls::Fraction length, scls::Fraction number_start, scls::Fraction number_end) {
        // Create the text
        ...
        // Create the number
        double line_start = number_start.to_double();
        double line_end = number_end.to_double();
        double step = 1;
    ...
    void Graphic_Object::update_event(double needed_delta_time) {
        ...
        if(is_focused() && is_overflighted() && is_clicked_during_this_frame(GLFW_MOUSE_BUTTON_LEFT)) {
            ...
            if(operation_at_click() == PLEOS_OPERATION_VECTOR) {
                ...
                add_vector(Point_2D(graphic_base_shared_ptr(), "a", movement_x.to_double(), movement_y.to_double()));
                ...
                // Create the point
                needed_point = new_point(std::string("point_") + std::to_string(objects().size()), movement_x.to_double(), movement_y.to_double());
                ...
                scls::Fraction movement_x = pixel_x_to_graphic_x(needed_x, texture()->image_shared_ptr());
                scls::Fraction movement_y = pixel_y_to_graphic_y(needed_y, texture()->image_shared_ptr());
                std::shared_ptr<Point_2D> needed_vector = std::make_shared<Point_2D>(graphic_base_shared_ptr(), std::string("Forme_") + std::to_string(current_form_2d()->points().size()), movement_x.to_double(), movement_y.to_double());
    ...
    int Graphic_Object::update_physic(double used_delta_time) {
        // Update the engine
        int needed_update = a_datas.get()->physic_engine()->update_physic_early(used_delta_time);

        // Collision events
        for(int i = 0;i<static_cast<int>(graphic()->physic_objects().size());i++) {
            __Graphic_Object_Base*object=graphic()->physic_object(i)->attached_object();
            if(object != 0) {
                for(int j = 0;j<static_cast<int>(graphic()->physic_objects().at(i).get()->current_collisions_results().size());j++) {
                    std::shared_ptr<Graphic_Collision_Event> needed_collision_event = std::make_shared<Graphic_Collision_Event>(graphic()->physic_objects().at(i).get()->current_collisions_results().at(j).get());
        ...
        // Late update
        needed_update += a_datas.get()->physic_engine()->update_physic_late(used_delta_time);
    ...
    void Graphic::load_circle_settings(scls::GUI_Object* object, pleos::Circle* needed_circle) {
        ...
        x->set_text(scls::Fraction::from_double(needed_circle->x()).to_std_string(0));
        ...
        y->set_text(scls::Fraction::from_double(needed_circle->y()).to_std_string(0));
    }
    void Graphic::save_circle_settings(scls::GUI_Object* object, pleos::Circle* needed_circle) {
        ...
        needed_circle->set_x(scls::Fraction::from_std_string(x->text()).to_double());
        needed_circle->set_y(scls::Fraction::from_std_string(y->text()).to_double());
    }
    ...
    void Graphic::load_form_settings(scls::GUI_Object* object, pleos::Form_2D* needed_form){
        ...
        x->attach_right_of_object_in_parent(x_title, 10);x->attach_bottom_of_object_in_parent(name_shared_ptr, 10);
        x->set_height_in_pixel(40);x->set_width_in_scale(scls::Fraction(1, 4));
        x->set_text(scls::Fraction::from_double(needed_form->x()).to_std_string(0));
        ...
        y->attach_right_of_object_in_parent(y_title, 10);y->attach_bottom_of_object_in_parent(name_shared_ptr, 10);
        y->set_height_in_pixel(40);y->set_width_in_scale(scls::Fraction(1, 4));
        y->set_text(scls::Fraction::from_double(needed_form->y()).to_std_string(0));
    }
    void Graphic::save_form_settings(scls::GUI_Object* object, pleos::Form_2D* needed_form){
        ...
        needed_form->set_x(scls::Fraction::from_std_string(x->text()).to_double());
        needed_form->set_y(scls::Fraction::from_std_string(y->text()).to_double());
    }
    ...
    std::string Graphic::to_xml_text() {
        ...
        if((!height_used() && !width_used()) || (height_used() && width_used())){to_return += std::string(" height=") + scls::Fraction::from_double(height()).to_std_string(0) + std::string(" width=") + scls::Fraction::from_double(width()).to_std_string(0);}
        else if(height_used()){to_return += std::string(" height=") + scls::Fraction::from_double(height()).to_std_string(0);}
        else if(width_used()){to_return += std::string(" width=") + scls::Fraction::from_double(width()).to_std_string(0);}
        to_return += std::string(" x=") + scls::Fraction::from_double(middle_x()).to_std_string(0) + std::string(" y=") + scls::Fraction::from_double(middle_y()).to_std_string(0);
        ...
        for(int i = 0;i<static_cast<int>(physic_objects().size());i++) {
            if(physic_object(i)->attached_object() !=0 && physic_object(i)->attached_object()->save_to_xml_text() && physic_object(i)->save_to_xml_text()) {
                //to_return += physic_object(i)->to_xml_text(); // TEMPORARY DISABLED
                if(i!=static_cast<int>(physic_objects().size())-1){to_return += std::string("\n");}
            }
        }
