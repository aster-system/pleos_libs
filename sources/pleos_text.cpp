...
namespace pleos {
	...
	void __graphic_from_xml_balise(std::shared_ptr<scls::XML_Text> xml, Text_Environment& environment, Graphic& graphic, std::shared_ptr<scls::Text_Style> text_style, scls::Fraction& graphic_height, scls::Fraction& graphic_width, scls::Fraction& graphic_x, scls::Fraction& graphic_y){
        ...
        if(current_balise_name == "arrow" || current_balise_name == "arrow_hat") {
            ...
            // Handle physic
            if(use_physic != 0){
                if(graphic.physic_map().size() <= 0){graphic.load_physic_map(0, 0);}
                std::shared_ptr<pleos::Graphic::Graphic_Physic> physic = std::make_shared<pleos::Graphic::Graphic_Physic>(circle.get()->attached_transform_shared_ptr());
                physic.get()->set_use_gravity(use_gravity);physic.get()->set_static(use_physic == 1);graphic.add_physic_object(physic);
